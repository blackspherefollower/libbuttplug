...
if [[ $1 == coverage ]]; then
  mkdir build
  cd build
  cmake .. -DCMAKE_BUILD_TYPE=Debug -DBPWS_BUILD_LIBWEBSOCKETS=ON -DBPWS_GENERATE_COVERAGE=ON $2
  cmake --build .
  ...
  mkdir build
  cd build
  cmake .. -DBPWS_BUILD_LIBWEBSOCKETS=ON $2
  cmake --build .
  cmake --build . --target test
elif [[ $1 == lxc ]]; then
  CONTAINER_NAME=libbpws-build-box-$3-$4-`date +%s`
  ...
  sudo lxc-attach -n $CONTAINER_NAME -- cmake -B/git_build -H/git_source -DBPWS_BUILD_LIBWEBSOCKETS=ON $2
